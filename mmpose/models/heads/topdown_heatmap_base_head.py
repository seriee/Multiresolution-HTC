...
#            print('img_metas:',img_metas[i])
        ...
        preds, maxvals = keypoints_from_heatmaps(
            ...
            kernel=self.test_cfg.get('modulate_kernel', 3),
    ...
    def decode_multiview(self, img_metas, output, **kwargs):
        """Decode keypoints from heatmaps.
        ...
        score = np.ones(batch_size)
        for i in range(batch_size):
            #print('img_metas:',batch_size)
            c[i, :] = img_metas[i]['c_2d'][0]
            s[i, :] = img_metas[i]['s_2d'][0]
            #print('c:',c)
            #print('s:',s)
            image_paths.append(img_metas[i]['image_file'])

            if 'bbox_score' in img_metas[i]:
                score[i] = np.array(img_metas[i]['bbox_score'][0]).reshape(-1)
            if bbox_ids is not None:
                bbox_ids.append(img_metas[i]['bbox_id'][0])
