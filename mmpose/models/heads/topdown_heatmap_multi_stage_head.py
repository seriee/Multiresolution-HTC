...
#            assert 'bbox_id' in img_metas[0]
        ...
        a_acc = 0
        for i in range(len(output)):
            output_point = TopdownHeatmapBaseHead.decode(self, img_metas, output[i].cpu().detach().numpy(),img_size=[img_width, img_height])
            ...
            a_acc += avg_acc
        accuracy['epe_pose'] = float(avg_acc/len(output))
        '''accuracy = dict()
