...
#            bias=bias)
        ...
        #### avgpool2d ######
#        self.projection = nn.AvgPool2d(kernel_size, stride=stride)
...
#            bias=True)
        #### maxpooling #####
#        self.projection = nn.MaxPool2d(kernel_size, stride=stride)
##        self.projection2 = nn.MaxPool2d(kernel_size=kernel_size,stride=stride,padding=2,dilation=2)
#        self.projection2 = nn.MaxPool2d(kernel_size=5, stride=2,padding=1)
#        self.conv1 = build_conv_layer(
#            dict(type=conv_type),
...
#            kernel_size=1,
#            bias=True)
        ###Depwise-SeperableConv####
#        self.projection = DepthwiseSeparableConvModule(
#            in_channels=in_channels,
#            out_channels=embed_dims,
#            kernel_size=kernel_size,
#            stride=stride,
...
#            bias=bias)
        ##### Max + Avg Pool #####
#        self.projection1 = nn.MaxPool2d(kernel_size, stride=stride)
#        self.projection2 = nn.AvgPool2d(kernel_size, stride=stride)
#        self.conv1 = build_conv_layer(
#            dict(type=conv_type),
#            in_channels=in_channels,
#            out_channels=embed_dims,
...
#        pat_embed1 = self.projection(x)
#        pat_embed2 = self.projection2(x)
##        pat_embed = torch.cat((pat_embed1,pat_embed2),1)
#        pat_embed = pat_embed1 + pat_embed2
##        x_2 = self.projection2(x)
##        x = x_1 + x_2
##        print('proj_x: ',x.shape)
#        pat_embed = self.conv1(pat_embed)
##        print('conv1: ',x.shape)
        
#        #### Multi kernel patch embedding #####
        pat_embed1 = self.conv3(x)
#        print('conv3: ',pat_embed1.shape)
        pat_embed2 = self.conv5(x)
        pat_embed = pat_embed1 + pat_embed2
#        print('conv5: ',pat_embed2.shape)
#        pat_embed = torch.cat((pat_embed1,pat_embed2),1)
#        pat_embed = self.conv1(pat_embed)
...
#        print('flatten : ',x.flatten(2).shape)
...
#        print('transpose : ',x.shape)
        ...
        ##### Position Embedding #####
#        pos_embed = self.avg_pool(x)
#        pos_embed =self.conv2(pos_embed) #pos_embed
#        pos_embed = pos_embed.flatten(2).transpose(1,2)
#        
#        out = pat_embed + pos_embed
#        out = self.drop(out)
