...
def multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False,multiview=False):
    ...
    for data in data_loader:
        ...
        if rank == 0:
            ...
            if multiview:
                #print('mutliview')
                nview = len(result)
                batch = len(result[0]['preds'])
                batch_size = int(len(next(iter(data.values())))//nview)
            else:
                batch_size = len(next(iter(data.values())))
                batch = 1
            #print('num_camera:',num_camera)
            for _ in range(batch_size * world_size*batch):
                prog_bar.update()
