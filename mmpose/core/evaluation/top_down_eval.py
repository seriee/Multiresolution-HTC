...
def _calc_distances(preds, targets, mask, normalize):
    ...
    #print('distance1: ',distances.T.shape)
    return distances.T
...
def keypoint_epe(pred, gt, mask):
    ...
    distance_valid = distances[distances != -1]
    mean = distance_valid.sum() / max(1, len(distance_valid))
    return mean

def keypoint_epe_point(pred, gt, mask, keypoint_info):
    """Calculate the end-point error.
    ...
    #print('dis_valid:',distance_valid.shape)
    num_point, num_img = distances.shape
    mean = distances.sum(axis=1)/max(1, num_img)
    print('\n')
    for i in range(len(keypoint_info)):
        print(keypoint_info[i]['name'],' = ',mean[i])
#    distance_valid = distances[distances != -1]
#    mean = distance_valid.sum() / max(1, len(distance_valid))
    return mean

def keypoint_epe_std(pred, gt, mask):
    """Calculate the end-point error.
