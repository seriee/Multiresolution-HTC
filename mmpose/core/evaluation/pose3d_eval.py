...
def keypoint_mpjpe(pred, gt, mask, alignment,pixel_space_xy,pixel_space_z):
    ...
    error = pred - gt
    #print('error_:',error.shape)
    #print('num_joint:',len(error[0]))
    num_joint = len(error[0])
    for i in range(len(error)):
        for j in range(num_joint):
            error[i][j][0] = error[i][j][0]*pixel_space_xy
            error[i][j][1] = error[i][j][1]*pixel_space_xy
            error[i][j][2] = error[i][j][2]*pixel_space_z
    
    error_mean = np.linalg.norm(error, ord=2, axis=-1)[mask].mean()

    return error_mean

def keypoint_mpjpe_std(pred, gt, mask,pixel_space_xy,pixel_space_z):
    """Calculate the mean per-joint position error (MPJPE) and the error after
    rigid alignment with the ground truth (P-MPJPE).
    ...
    error_std = np.linalg.norm(error, ord=2, axis=-1)[mask]
    error_std = np.std(error_std)
    
    return error_std


def keypoint_3d_pck(pred, gt, mask,pixel_space_xy,pixel_space_z,alignment='none', threshold=0.15):
    ...
    for i in range(len(error)):
        for j in range(num_joint):
            ...
            error[i][j][1] = error[i][j][1]*pixel_space_xy
            error[i][j][2] = error[i][j][2]*pixel_space_z
    error = np.linalg.norm(error, ord=2, axis=-1)
