...
from mmpose.datasets.pipelines.top_down_transform import Gernerate_Target2d_Multiview
from mmpose.models.utils.filters import ram_lak_3D
...
class NormalizeJointCoordinate:
    ...
    def __call__(self, results):
       ...
       # print('results:',results[self.item].shape)
...
@PIPELINES.register_module()
class GenerateMeanRef_pyronn:
    """Generate the target 3d Mean Reference point ground truth.

    Required keys: 'joint3d'.
    Modified keys: 'target_3d', and 'target_weight_3d'. [Mean reference target]
    ...
    def __init__(self,
                 ...
                 num_camera=1,
                 space_size=[400,400,400],
                 space_center=[200,200,200]):
        self.sigma = sigma
        ...
        self.heatmap2d_gen = Gernerate_Target2d_Multiview(sigma=self.sigma)
    ...
    def __call__(self, results):
        ...
        cfg = results['ann_info']
        camera_param = results['cameras']
        num_joints = joints_2d[0].shape[0]
        height = results['height_2d'][0]
        width = results['width_2d'][0]
        # print('num_joints:',num_joints)
        # print('joint_2d:',len(joints_2d))
        # print('results:',len(results))
        heatmaps_list = []
        for i in range(self.num_camera):
            heatmaps = self.heatmap2d_gen(cfg,joints_2d[i],joints_2d_visible[i])
            heatmaps_list.append(heatmaps)
        heatmaps_ = np.stack(heatmaps_list,axis=0)
        heatmaps_3d = []
        for j in range(num_joints):
            projection_matrices = np.zeros((self.num_camera, 3, 4))  
            projection_images = heatmaps_[:,j,:,:]
            #print('R:',camera_param[0]['R'].shape, 'T:',camera_param[0]['T'].shape)
            #print('RT:',np.concatenate((camera_param[x]['R'], camera_param[x]['T']), dim=1))
            for x in range(self.num_camera):
                projection_matrices[x, ...] = camera_param[x]['K'] @ np.concatenate((camera_param[x]['R'], camera_param[x]['T']), axis=1)
            #print('projection_matrices:',projection_matrices.shape)
            # ramlak filtering using PYRO-NN implementation of ramlak filter in Fourier domain
            geometry = {'detector_shape': (height, width),
                    'detector_spacing': (1.0, 1.0),
                    'number_of_projections': self.num_camera}
            geometry = SimpleNamespace(**geometry)
            ram_lak_filter = ram_lak_3D(geometry)
            n = projection_images.shape[-1]
            ram_lak_filter = ram_lak_filter[:, :, :int(np.floor(n / 2) + 1)]
            projection_images = np.fft.rfft(projection_images)
            ...
            projector = pyronn_torch.ConeBeamProjector(
                        self.space_size,  # volume shape
                        (1.0, 1.0, 1.0),  # volume spacing in mm
                        self.space_center,  # volume origin in mm
                        (self.num_camera, height, width),  # projection_shape (number of projections, height, width)
                        (1, 1),  # projection_spacing in mm - only relevant for intensity scaling
                        (0, 0),  # projection origin in mm - irrelevant for backprojection
                        projection_matrices
            )
            projection_images = torch.from_numpy(projection_images)
            reconstruction = projector.project_backward(projection_images, use_texture=True)
            heatmaps_3d.append(reconstruction)
        target_3d = torch.stack(heatmaps_3d,dim=0)
        #print('target_3d:',target_3d.shape)
        results['targets_3d'] = target_3d.cpu()
        results['projection_matrices'] = results['cameras']
