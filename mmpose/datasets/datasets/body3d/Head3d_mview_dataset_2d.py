...
from mmpose.datasets.datasets.base import Kpt3dMviewRgbImgDirectDataset,Kpt2dSviewRgbImgTopDownDataset
...
class Head3DMviewDataset(Kpt2dSviewRgbImgTopDownDataset):
    """
    Args:
    ...
    def _get_db(self):
        ...
        for i in range(num_sequence):
            ...
            for k, cam_param in cam_params.items():
                ...
                #print('k:',k)
                #print('img_file_:',self.image[k]['file_name'])
                #img_file = self.img_prefix+self.image[k]['file_name']
                #print('cam_param:',cam_param)
                img_ann = self.coco.loadImgs(k)[0]
                width = img_ann['width']
                height = img_ann['height']
                num_joints = self.ann_info['num_joints']
                ann_ids = self.coco.getAnnIds(imgIds=k, iscrowd=False)
                ...
                for obj in objs:
                    ...
                    if 'num_keypoints' in obj and obj['num_keypoints'] == 0:
                        continue
                    joints_2d = np.zeros((num_joints, 3), dtype=np.float32)
                    joints_2d_visible = np.zeros((num_joints, 3), dtype=np.float32)

                    keypoints = np.array(obj['keypoints']).reshape(-1, 3)
                    joints_2d[:, :2] = keypoints[:, :2]
                    joints_2d_visible[:, :2] = np.minimum(1, keypoints[:, 2:3])
                    c_2d, s_2d = self._xywh2cs(0, 0, width, height)#, padding=1)

                    image_file = os.path.join(self.img_prefix, self.id2name[k])
                    #print('image_file:',image_file)
                    pose_3d = np.array([annotation[k]['keypoint_3d']]).astype('float32')
                    #print('pose_3d:',pose_3d.shape)
                    ...
                    rec.append({
                        ...
                        'person_ids':0,
                        'joints_2d': joints_2d,
                        'joints_2d_visible': joints_2d_visible,
        ...
        for output in outputs:
            ...
            for x in range(nview):
                ...
                for i in range(batch_size):
                    image_id = self.name2id[image_paths[i][x].split('/')[-1]]
...
#        print('kpts:',len(kpts))
    ...
    def __getitem__(self, idx):
        ...
        for c in range(self.num_cameras):
           ...
           # print(self.num_cameras * idx + c)
            result = copy.deepcopy(self.db[self.num_cameras * idx + c])
            result['ann_info'] = self.ann_info
            height = result['ann_info']['image_size'][0]
            width = result['ann_info']['image_size'][1]
